...
需要选择使用 AgentScope 1.0 框架和百炼平台的 qwen3-max 模型。选手可以选择：

- 在现有的 `agentscope.agent.ReActAgent` 基础上进行开发
- 继承 `agentscope.agent.ReActAgentBase` 进行开发
- 继承 `agentscope.agent.AgentBase` 进行开发
...
> **注意**：由于本赛题考察Agent的能力，为避免模型差异带来的影响，参赛选手的智能体只能使用百炼提供的 "qwen3-max" 模型，但模型参数不做限制（temperature等）。阿里云百炼平台提供了每个模型100万免费Tokens，详情参考：如何使用阿里云百炼上的模型？
...
参赛选手的智能体将持续参赛，即多局狼人杀游戏之间智能体状态是连续的，在一局比赛开始前将加载状态，结束后将保存其状态，具体而言：

1. 采取随机分组的方式，每组 9 位参赛者，最后一组不满 9 人时，从其他参赛选手中随机抽取补充至 9 人，补充选手的胜负不计入其成绩
2. 一组固定的选手将进行 5 局游戏，每局游戏开始时都会重新分配身份
3. 一组选手进行完 5 局比赛视为 1 轮，共进行 10 轮
4. 比赛最开始，选手的智能体将会被赋予一个名字，该名字将是固定的，不同游戏局次之间不会产生变化
5. 智能体每次发言的时间（到结束发言为止）不能超过 30s，同时产生的文字结果字符串长度不能超过 2048，否则跳过发言，在投票阶段则算作弃权
6. 提示词注入攻击是被允许的，禁止选手采取除"智能体信息交流"以外的任何攻击手段，例如代码层面的攻击，一经发现取消比赛资格
